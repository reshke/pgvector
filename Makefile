# Both names supported: vector (canonical) and pgvector (alias for PGaaS compatibility)
...
# Create pgvector.* symlinks for CREATE EXTENSION pgvector compatibility
pgvector-symlinks:
	ln -sf vector.control pgvector.control
	@for f in sql/vector--*.sql; do \
	  base=$$(basename $$f); \
	  dest="sql/pgvector--$${base#vector--}"; \
...
sql/$(EXTENSION_BUILD)--$(EXTVERSION).sql: sql/$(EXTENSION_BUILD).sql | pgvector-symlinks
	cp $< $@

sql/pgvector--$(EXTVERSION).sql: sql/$(EXTENSION_BUILD)--$(EXTVERSION).sql
